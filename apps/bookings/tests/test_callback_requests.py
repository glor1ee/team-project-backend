...
def test_create_callback_request_minimal(client):
    response = client.post(
        "/api/callback-requests/", {"phone": "+380501234567"}, content_type="application/json"
    )
...
def test_create_callback_request_with_equipment_and_dates(client):
    ...
    equipment = Equipment.objects.create(
        ...
        category=category, price_per_day="650.00",
    )
    response = client.post(
...
def test_phone_is_required(client):
    response = client.post("/api/callback-requests/", {}, content_type="application/json")
